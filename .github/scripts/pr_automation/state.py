...
from .utils import (
    MSG_INITIAL_TEMPLATE_SETUP,
    MSG_TEMPLATE_NOT_FOUND,
    MSG_TEMPLATE_UNCHANGED,
    OP_CHECK,
    OP_CHECK_CHANGED,
    OP_MARK_CHECKED,
    OP_UPDATE_STATE,
...
class StateManager:
    ...
    def manage_organization(
        self, org_name: str, current_repo_count: int, operation: str = OP_CHECK
    ) -> tuple[bool, str]:
        ...
        if operation == OP_CHECK:
            ...
            if current_repo_count > stored_count:
                return (
                    True,
                    f"New repositories detected: {stored_count} → {current_repo_count} (+{current_repo_count - stored_count})",
                )
            if current_repo_count < stored_count:
                ...
                return (
                    False,
                    f"Repositories deleted: {stored_count} → {current_repo_count} (-{stored_count - current_repo_count}), no scan needed",
                )

            return False, f"No changes detected ({current_repo_count} repos)"

        if operation == OP_MARK_CHECKED:
            # Mark organization as checked and update state
            now = datetime.now().isoformat()
            if org_name not in state["organizations"]:
                state["organizations"][org_name] = {}

            updates = {"repo_count": current_repo_count, "last_check": now}
    ...
    def manage_template(self, operation: str, new_hash: str = None) -> tuple[bool, str]:
        ...
        if operation == OP_UPDATE_STATE:
            ...
            state["template_state"].update(
                {"workflow_yml_hash": new_hash, "last_template_update": datetime.now().isoformat()}
            )
